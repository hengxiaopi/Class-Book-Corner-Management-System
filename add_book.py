...
class Ui_Form:
    def setupUi(self, Form):
        ...
        self.number = QtWidgets.QSpinBox(Form)
        self.number.setGeometry(QtCore.QRect(70, 10, 81, 31))
        self.number.setMinimum(1000)
        ...
        self.count.setProperty("value", 1)
    ...
    def commit_info(self):
        ...
        if self.classify.text() and self.name.text() and self.author.text() and self.publisher.text():
            if self.number.text() not in data:
                self.finish=True
                with open("add_info.txt","w",encoding="utf-8") as f:
                    f.write(str([self.number.text(),self.classify.text(),self.name.text(),self.author.text(),self.publisher.text(),self.date.text(),self.count.text()]))
                self.form.close()
            else:
                window=tk.Tk()
                window.withdraw()
                messagebox.showwarning("班级图书角管理系统","编号已使用，请重新输入编号")
        else:
            window=tk.Tk()
            window.withdraw()
...
def main():
    ...
    app.exec_()
    if not ui.finish:
        open("add_info.txt","w",encoding="utf-8").write("None")
